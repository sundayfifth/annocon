...
// Symmetric with OUTSIDE_MARGIN: 20px clear of the frame being annotated,
// 20px clear of whatever frame is next door — so a 160px gap between two
// frames lands the card at exactly its 120px floor width with no bleed on
// either side.
export const NEIGHBOR_SAFETY_GAP = 20
 ...
 */
export function floorFor(metrics: CardMetrics): number {
 ...
 *
 * `gap` is the distance to the nearest frame on the side the card is going,
 * which only the scene layer can measure; `Infinity` means nothing is next
 * door and the card keeps the width its size asks for.
 */
export function shrinkToFit(metrics: CardMetrics, gap: number): number {
